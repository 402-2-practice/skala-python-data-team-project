...
def main() -> None:
    ...
    print("=" * 60)
    print("[EDA] Adult Census 탐색적 데이터 분석 완료")
    print("=" * 60)
    print(
        "정제 표본 수:",
        f"{summary['dataset']['rows']:,}",
    )
    print(
        "전체 고소득률:",
        (
            f"{summary['target_distribution']['high_income_rate']:.2%}"
        ),
    )

    degree_rate = association[
        "college_degree_high_income_rate"
    ]
    non_degree_rate = association[
        "no_college_degree_high_income_rate"
    ]
    rate_difference = association[
        "high_income_rate_difference"
    ]

    if degree_rate is not None:
        print(
            "대학 학위 집단 고소득률:",
            f"{degree_rate:.2%}",
        )

    if non_degree_rate is not None:
        print(
            "비학위 집단 고소득률:",
            f"{non_degree_rate:.2%}",
        )
